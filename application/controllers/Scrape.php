...
class Scrape extends CI_Controller
{
	...
	function SyncMpinfoData(){
			...
			//$query = "SELECT * FROM `mpinfo_xml_news` where id=1294"; 
			...
			$newsImage = $this->scrape_between($newsDescription, "src='", "style='cursor: pointer;'  />");
			if($newsImage){		
			$newsImage = str_replace("'","",$newsImage);			
			$newsImage = trim($newsImage);
			$imageUrl = $newsImage;
			$newsDescription = str_replace("<div class='news_img'>  <img data-toggle='modal' data-target='#myModal'  onclick='image(this)'  class='img-fluid' alt=''  src='$newsImage'  style='cursor: pointer;'  /> </div>","",$newsDescription);
	...
	//function saveNewsByLink() {
	function saveNewsByLink($url,$section=7) { 	
		//$section=2;
		//$url = "https://galavnews.com/news.php?id=president-murmus-address-in-the-joint-session-of-parliament-congratulations-to-om-birla-and-election-commission-497148";
		//$url = "https://galavnews.com/news.php?id=finance-minister-can-make-a-new-announcement-regarding-vande-bharat-in-the-budget-499982";
		//$url= 'https://galavnews.com/news.php?id=taliban-did-not-allow-14-lakh-girls-to-go-to-school-505454';
			...
			$newsImage = str_replace('"',"",$newsImage);			
			$newsImage = trim($newsImage);
			$newsDescription = str_replace('<img src="'.$newsImage.'" class="post-image img-responsive " />',"",$newsDescription);    
			}

			
			$newsImage = $this->scrape_between($newsDescription, '<img src="', 'class="post-image img-responsive pull-left" />');	
			if($newsImage){		
			$newsImage = str_replace('"',"",$newsImage);			
			$newsImage = trim($newsImage);
			$newsDescription = str_replace('<img src="'.$newsImage.'" class="post-image img-responsive pull-left" />',"",$newsDescription);
			}
			//pr($newsDescription);
			...
			$newsDescription =trim($newsDescription);
			$newsDescription =addslashes($newsDescription);  
			  //  save new 
			  $newsData  = array();
	...
	function test(){
		die();
				...
				curl_setopt($curl_handle,CURLOPT_URL,'https://mpinfo.org/RSSFeed/RSSFeed_News.xml');
				...
				echo $error_msg = curl_error($curl_handle);
				}
				die();
	...
	function SyncMpinfoDataTest(){
		
		/*$imageUrl = 'https://www.mpinfo.org/mpinfonew/NewsImages/Bhopal/Wednesday, August 21, 2024/TN5-Bhopal210824082719.jpg';
		...
		$saveImageName = time().'.jpg';
		$saveImageNamePath = 'uploads/'.$saveImageName;
		$status = file_put_contents($saveImageNamePath, $image);
		var_dump($status);*/
